...
public class PermissionAllowDenyTests
{
    ...
    public PermissionAllowDenyTests(SharedWebApiHost sharedHost, ITestOutputHelper output)
    {
    ...
    {
        _output.WriteLine("[TEST] AllowGrant_GivesAccessWithoutRole");
        ...
        _output.WriteLine("[STEP] Verifying user CANNOT access other user without permission...");
        ...
        _output.WriteLine($"[RECEIVED] Before grant: {(int)beforeResp.StatusCode} {beforeResp.StatusCode}");
        Assert.Equal(HttpStatusCode.Forbidden, beforeResp.StatusCode);

        // Grant Allow permission directly to the user (as admin)
        _output.WriteLine("[STEP] Granting Allow permission to user...");
        ...
        _output.WriteLine($"[RECEIVED] Grant: {(int)grantResp.StatusCode} {grantResp.StatusCode}");
        ...
        _output.WriteLine("[STEP] Verifying user CAN access target user after Allow grant...");
        ...
        _output.WriteLine($"[RECEIVED] After grant: {(int)afterResp.StatusCode} {afterResp.StatusCode}");
        Assert.Equal(HttpStatusCode.OK, afterResp.StatusCode);

        _output.WriteLine("[PASS] Allow grant gives access without requiring a role");
    }
    ...
    {
        _output.WriteLine("[TEST] RevokeAllowGrant_RemovesAccess");
        ...
        _output.WriteLine("[STEP] Granting Allow permission...");
        var grantRequest = new
        {
            UserId = regularUserId,
            PermissionIdentifier = $"api:iam:users:read;userId={targetUserId}",
        ...
        grantReq.Headers.Authorization = new AuthenticationHeaderValue("Bearer", adminAuth!.AccessToken);
        grantReq.Content = JsonContent.Create(grantRequest);
        await _sharedHost.Host.HttpClient.SendAsync(grantReq);
        ...
        _output.WriteLine("[STEP] Revoking the Allow grant...");
        ...
        var revokeResp = await _sharedHost.Host.HttpClient.SendAsync(revokeReq);
        _output.WriteLine($"[RECEIVED] Revoke: {(int)revokeResp.StatusCode} {revokeResp.StatusCode}");
        ...
        _output.WriteLine("[STEP] Verifying access is denied after revocation...");
        ...
        _output.WriteLine($"[RECEIVED] After revoke: {(int)accessResp2.StatusCode} {accessResp2.StatusCode}");
        Assert.Equal(HttpStatusCode.Forbidden, accessResp2.StatusCode);

        _output.WriteLine("[PASS] Revoking Allow grant removes access");
    }
    ...
    {
        _output.WriteLine("[TEST] DenyGrant_BlocksAccessDespiteRole");
        ...
        _output.WriteLine("[STEP] Creating role with api:iam:users:read permission...");
        ...
        _output.WriteLine("[STEP] Assigning role to user...");
        ...
        _output.WriteLine("[STEP] Verifying user CAN access with role...");
        using var accessReq1 = new HttpRequestMessage(HttpMethod.Get, $"/api/v1/iam/users/{targetUserId}");
        accessReq1.Headers.Authorization = new AuthenticationHeaderValue("Bearer", userWithRole!.AccessToken);
        var accessResp1 = await _sharedHost.Host.HttpClient.SendAsync(accessReq1);
        _output.WriteLine($"[RECEIVED] With role: {(int)accessResp1.StatusCode} {accessResp1.StatusCode}");
        Assert.Equal(HttpStatusCode.OK, accessResp1.StatusCode);

        // Now grant a DENY for this specific target user
        _output.WriteLine("[STEP] Granting Deny for specific target user...");
        var denyRequest = new
        {
            UserId = regularUserId,
            PermissionIdentifier = $"api:iam:users:read;userId={targetUserId}",
        ...
        _output.WriteLine($"[RECEIVED] Deny grant: {(int)denyResp.StatusCode} {denyResp.StatusCode}");
        ...
        _output.WriteLine("[STEP] Verifying user CANNOT access target despite role (Deny overrides)...");
        using var accessReq2 = new HttpRequestMessage(HttpMethod.Get, $"/api/v1/iam/users/{targetUserId}");
        accessReq2.Headers.Authorization = new AuthenticationHeaderValue("Bearer", userWithDeny!.AccessToken);
        var accessResp2 = await _sharedHost.Host.HttpClient.SendAsync(accessReq2);
        _output.WriteLine($"[RECEIVED] With Deny: {(int)accessResp2.StatusCode} {accessResp2.StatusCode}");
        Assert.Equal(HttpStatusCode.Forbidden, accessResp2.StatusCode);
        ...
        _output.WriteLine("[STEP] Verifying user CAN still access other users (deny is specific)...");
        ...
        _output.WriteLine($"[RECEIVED] Access other user: {(int)accessResp3.StatusCode} {accessResp3.StatusCode}");
        Assert.Equal(HttpStatusCode.OK, accessResp3.StatusCode);

        _output.WriteLine("[PASS] Deny grant blocks access despite role, but only for the specific resource");
    }
    ...
    {
        _output.WriteLine("[TEST] RevokeDenyGrant_RestoresRoleAccess");
        ...
        createRoleReq.Headers.Authorization = new AuthenticationHeaderValue("Bearer", adminAuth!.AccessToken);
        createRoleReq.Content = JsonContent.Create(createRoleRequest);
        await _sharedHost.Host.HttpClient.SendAsync(createRoleReq);
        ...
        denyReq.Headers.Authorization = new AuthenticationHeaderValue("Bearer", adminAuth.AccessToken);
        denyReq.Content = JsonContent.Create(denyRequest);
        await _sharedHost.Host.HttpClient.SendAsync(denyReq);
        ...
        // Revoke the deny
        _output.WriteLine("[STEP] Revoking the Deny grant...");
        var revokeRequest = new
        {
            UserId = regularUserId,
            PermissionIdentifier = $"api:iam:users:read;userId={targetUserId}"
        ...
        _output.WriteLine("[STEP] Verifying access is restored via role after revoking Deny...");
        using var accessReq2 = new HttpRequestMessage(HttpMethod.Get, $"/api/v1/iam/users/{targetUserId}");
    ...
    {
        _output.WriteLine("[TEST] GlobalDeny_OverridesAllRolePermissions");
        ...
        Assert.Equal(HttpStatusCode.OK, (await _sharedHost.Host.HttpClient.SendAsync(access1)).StatusCode);

        // Grant a GLOBAL deny (no userId parameter means all users)
        _output.WriteLine("[STEP] Granting global Deny for api:iam:users:read...");
        ...
        var globalDenyResp = await _sharedHost.Host.HttpClient.SendAsync(globalDenyReq);
        _output.WriteLine($"[RECEIVED] Global Deny: {(int)globalDenyResp.StatusCode} {globalDenyResp.StatusCode}");
        ...
        _output.WriteLine("[STEP] Verifying user CANNOT access any user with global Deny...");
        ...
        var resp2a = await _sharedHost.Host.HttpClient.SendAsync(access2a);
        _output.WriteLine($"[RECEIVED] Access user1: {(int)resp2a.StatusCode}");
        ...
        var resp2b = await _sharedHost.Host.HttpClient.SendAsync(access2b);
        _output.WriteLine($"[RECEIVED] Access user2: {(int)resp2b.StatusCode}");
        Assert.Equal(HttpStatusCode.Forbidden, resp2b.StatusCode);

        _output.WriteLine("[PASS] Global Deny blocks access to all resources of that type");
    }
    ...
    {
        _output.WriteLine("[TEST] PermissionGrants_RequireReLoginToTakeEffect");
        _output.WriteLine("Testing that direct permission grants require re-login (they are baked into JWT)");
        ...
        _output.WriteLine("[STEP] Verifying CANNOT access with original token...");
        ...
        var resp1 = await _sharedHost.Host.HttpClient.SendAsync(access1);
        ...
        _output.WriteLine("[STEP] Verifying STILL CANNOT access using SAME token (grant not in token yet)...");
        ...
        var resp2 = await _sharedHost.Host.HttpClient.SendAsync(access2);
        _output.WriteLine($"[RECEIVED] After grant (same token): {(int)resp2.StatusCode}");
        Assert.Equal(HttpStatusCode.Forbidden, resp2.StatusCode);

        // Re-login to get a new token with the grant
        _output.WriteLine("[STEP] Re-logging in to get new token with grant...");
        ...
        _output.WriteLine("[STEP] Verifying CAN access using NEW token (after re-login)...");
        ...
        var resp3 = await _sharedHost.Host.HttpClient.SendAsync(access3);
        _output.WriteLine($"[RECEIVED] After re-login: {(int)resp3.StatusCode}");
        Assert.Equal(HttpStatusCode.OK, resp3.StatusCode);

        _output.WriteLine("[PASS] Permission grants require re-login to take effect (by design)");
    }
    ...
    {
        _output.WriteLine("[TEST] SpecificDeny_OverridesBroadAllow");
        ...
        _output.WriteLine("[STEP] Granting specific Deny for blocked user...");
        var denyRequest = new
        {
            UserId = regularUserId,
        ...
        _output.WriteLine("[STEP] Verifying CAN access non-blocked user...");
        ...
        var respAllowed = await _sharedHost.Host.HttpClient.SendAsync(accessAllowed);
        _output.WriteLine($"[RECEIVED] Access allowed user: {(int)respAllowed.StatusCode}");
        Assert.Equal(HttpStatusCode.OK, respAllowed.StatusCode);

        // Verify: CANNOT access blocked user (specific deny overrides)
        _output.WriteLine("[STEP] Verifying CANNOT access blocked user...");
        ...
        var respBlocked = await _sharedHost.Host.HttpClient.SendAsync(accessBlocked);
        _output.WriteLine($"[RECEIVED] Access blocked user: {(int)respBlocked.StatusCode}");
        Assert.Equal(HttpStatusCode.Forbidden, respBlocked.StatusCode);

        _output.WriteLine("[PASS] Specific Deny correctly overrides broad Allow from role");
    }
